...
public class ProjectRecordBatch extends AbstractSingleRecordBatch<Project> {
  ...
  private void setupNewSchemaFromInput(RecordBatch incomingBatch) throws SchemaChangeException {
    ...
    this.outputExpressions = Lists.newArrayList();
    if (complexWriters != null) {
      container.clear();
    } else {
    ...
    for (NamedExpression namedExpression : exprs) {
      ...
      if (classify && namedExpression.getExpr() instanceof SchemaPath) {
        ...
        if (handleWildCardExpressions(incomingBatch, collector, transfers, cg, result)) {
          continue;
        }
      } else {
...
//        final String outputName = getRef(namedExpression).getRootSegment().getPath(); //moved to before the if
      ...
      if (expr instanceof ValueVectorReadExpression
        ...
        setupTransferPair(incomingBatch, transfers, transferFieldIds, namedExpression, (ValueVectorReadExpression) expr);
